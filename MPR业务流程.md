...
## ⑭ 测量（F1/F2）：线段 / 矩形 / 椭圆 / 角度 / 曲线 / 自由形状 + HU 统计

| 工具 | 操作 | 结束 | 输出 |
| --- | --- | --- | --- |
| 线段 | 2 点 | 自动 | 长度 mm |
| 矩形 | 2 点（对角） | 自动 | 面积 cm² + HU 均值(min,max,n) |
| 椭圆 | 2 点（外接矩形对角） | 自动 | 面积 cm² + HU 统计 |
| 角度 | 3 点（第 2 点为顶点） | 自动 | 角度 ° |
| 曲线 | 多点折线 | **双击 / Enter** | 总长度 mm（+点数，不做 HU） |
| 自由形状 | 按住左键拖动描画 | 松开闭合 | 面积 cm² + 周长 mm + HU 统计 |

**工具条（第 2 行）**：`测量:` 下拉（6 个工具 + `删除选中` + `全部删除`）· `十字线/平移` 切换按钮 · `重置` · `回正` · 结果 · 提示。
`Esc` 退出测量回十字线模式；`Delete` 删除选中；完成一条测量后**自动选中**它。

**选中**：十字线模式下左键点中测量（≤6px）即选中并高亮（加粗+亮色），**命中测量优先于跳转中心**；点空白取消选中。

**斜切与旋转（Q7）**：斜切下**允许测量**；**转动某视图时该视图的测量保持不动，其他视图的测量全部清空**
（其他视图平面法向已变，测量不再有效）。测量平面基准取自各视图**相机**（`ScreenFrame` 的右/上方向），
ROI 取样按"平面 mm 坐标 → 世界 → 体素索引 → 三线性取值"，因此斜切下面积与 HU 统计依然正确。
